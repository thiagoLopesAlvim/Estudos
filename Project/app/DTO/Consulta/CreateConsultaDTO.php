...
class CreateConsultaDTO{
    ...
    public string $endereco, public string $observacao, public string $tippagamento,public string $dtnascimento, public string $dtconsulta){
    }
    ...
    public static function makeFromRequest(StoreConsultaRequest $request){
        return new self($request->tipconsulta, 'a', $request->nomecliente, $request->telefone,$request->CPF,
                        $request->endereco,$request->observacao,$request->tippagamento,$request->dtnascimento, $request->dtconsulta);
    }
}